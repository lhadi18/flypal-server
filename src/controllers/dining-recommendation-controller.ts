...
export const deleteRecommendation = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    // Delete associated bookmarks for the dining recommendation
    await Bookmark.deleteMany({ diningId: id, sourceType: 'DINING_USER_POST' })

    // Delete the dining recommendation image from Google Cloud Storage (if it exists)
    ...
    // Delete the dining recommendation
    await DiningRecommendation.deleteOne({ _id: id })

    res.status(200).json({ message: 'Recommendation and associated bookmarks deleted successfully' })
  } catch (error) {
